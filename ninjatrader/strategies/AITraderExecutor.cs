...
namespace NinjaTrader.NinjaScript.Strategies
{
    public class AITraderExecutor : Strategy
    {
        ...
        // Pattern tracking
        private string currentPatternType = "NONE"; // Tipo de patrón detectado
        private double patternConfidence = 0.0;     // Confianza en el patrón detectado (0-1)
        private double riskRewardRatio = 1.0;       // Ratio riesgo-recompensa del trade
        #endregion
        ...
        [NinjaScriptProperty]
        [Display(Name = "Max Risk Percent", Description = "Maximum risk per trade as percentage of account", Order = 8, GroupName = "Risk Management")]
        public double MaxRiskPercent { get; set; }
        
        [NinjaScriptProperty]
        [Display(Name = "Min Risk/Reward", Description = "Minimum risk to reward ratio for trades", Order = 9, GroupName = "Risk Management")]
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description = "AI Trading strategy that uses pattern recognition and NeurEvo for reversal pattern trading";
                Name = "AITraderExecutor";
                ...
                BasePositionSize = 1;
                OrderRetryCount = 3;
                UseManagedOrders = true;
                MaxRiskPercent = 1.0;   // 1% del capital por trade
                MinRiskReward = 1.5;    // Ratio riesgo-recompensa mínimo
        ...
        private void ProcessTradingSignal(string message)
        {
            ...
            {
                Print("Invalid order message format. Expected at least 7 parameters.");
            ...
            if (!int.TryParse(commandParts[0], out tradeSignal) ||
                !double.TryParse(commandParts[1], out positionSize) ||
                !double.TryParse(commandParts[2], out stopLoss) ||
                !double.TryParse(commandParts[3], out takeProfit) ||
                !double.TryParse(commandParts[5], out confidence) ||
                !double.TryParse(commandParts[6], out riskReward))
            {
            ...
            patternType = commandParts[4];
            
            // Validaciones adicionales con control de riesgo
            ...
            if (riskReward < MinRiskReward && tradeSignal != 0)
            {
                Print($"Risk/Reward ratio too low: {riskReward}, minimum required: {MinRiskReward}");
                orderServer.SendMessage("ERROR:Risk_Reward_Too_Low\n");
                return;
            }
            ...
            Print($"Received trade signal: Signal={tradeSignal}, Size={positionSize}, " +
                  $"SL={stopLoss.ToString("F2")}, TP={takeProfit.ToString("F2")}, " +
                  $"Pattern={patternType}, Confidence={confidence.ToString("F2")}, R/R={riskReward.ToString("F2")}");
            
            // Ejecutar la decisión de trading
            ExecuteTradingDecision();
            
            // Confirmar que el mensaje se procesó correctamente
            orderServer.SendMessage($"ORDER_CONFIRMED:{tradeSignal},{positionSize},{patternType},{confidence}\n");
        }
        #endregion
        ...
        protected override void OnExecutionUpdate(Execution execution, string executionId, double price, int quantity, MarketPosition marketPosition, string orderId, DateTime time)
        {
            try
            {
                ...
                {
                    // Este es un pedido de salida
                    if (orderId.Contains("Long"))
                    {
                        action = "Exit Long";
                        exitPrice = price;
                        
                        // Calcular P&L para posición larga
                    ...
                    else if (orderId.Contains("Short"))
                    {
                        ...
                        // Calcular P&L para posición corta
                        double positionEntryPrice = Position.AveragePrice;
                        pnl = (positionEntryPrice - exitPrice) * quantity * Instrument.MasterInstrument.PointValue;
                    }
                }
                
                // Send execution data to Python with pattern information
                if (!string.IsNullOrEmpty(action))
                {
                    string executionMessage = $"TRADE_EXECUTED:{action},{entryPrice},{exitPrice},{pnl},{quantity},{currentPatternType},{patternConfidence},{riskRewardRatio}\n";
                    orderServer.SendMessage(executionMessage);
                    Print($"Sent trade execution data: {action}, P&L: {pnl}, Pattern: {currentPatternType}");
                }
            }
            catch (Exception ex)
            {
